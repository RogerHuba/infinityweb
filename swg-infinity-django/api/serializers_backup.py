...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'last_name', 'is_active', 'date_joined', 'last_login', 'password'
        ]
        extra_kwargs = {
    ...
    def get_total_playtime_hours(self, obj):
        """Convert total playtime to hours for easier reading"""
...
class UserPublicSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = [
            'id', 'username', 'character_name', 'is_online', 
            'created_at', 'total_playtime_hours'
        ]
...
class ServerStatusSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ServerStatus
        fields = [
            'id', 'server_name', 'status', 'player_count', 'max_players',
            'uptime', 'uptime_hours', 'last_restart', 'cpu_usage', 
            'memory_usage', 'message_of_the_day', 'created_at', 
            'updated_at', 'player_percentage'
        ]
    ...
    def get_player_percentage(self, obj):
        """Calculate player percentage of max capacity"""
...
class ServerConfigurationSerializer(serializers.ModelSerializer):
    """Serializer for ServerConfiguration model"""
...
class GameSessionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = GameSession
        fields = [
            'id', 'user', 'user_username', 'character_name', 'login_time', 
            'logout_time', 'is_active', 'ip_address', 'location_x', 
            'location_y', 'location_z', 'planet', 'session_duration_minutes',
            'is_currently_active'
        ]
...
class GameSessionCreateSerializer(serializers.ModelSerializer):
    """Simplified serializer for creating game sessions"""
    
    class Meta:
        model = GameSession
        fields = [
            'user', 'character_name', 'ip_address', 'location_x', 
            'location_y', 'location_z', 'planet'
        ]

class PlayerStatsSerializer(serializers.ModelSerializer):
    """Serializer for PlayerStats model"""
    user_username = serializers.CharField(source='user.username', read_only=True)
    user_character_name = serializers.CharField(source='user.character_name', read_only=True)
    total_playtime_hours = serializers.SerializerMethodField()
    ...
    class Meta:
        model = PlayerStats
        fields = [
            'id', 'user', 'user_username', 'user_character_name', 
            'total_sessions', 'total_playtime', 'total_playtime_hours', 
            'last_seen', 'favorite_planet', 'credits_earned', 
            'experience_points', 'level', 'guild_name', 'pvp_kills', 
            'pvp_deaths', 'kd_ratio', 'created_at', 'updated_at'
        ]
...
class PlayerStatsPublicSerializer(serializers.ModelSerializer):
    """Public serializer for PlayerStats (leaderboard view)"""
    user_character_name = serializers.CharField(source='user.character_name', read_only=True)
    kd_ratio = serializers.ReadOnlyField()
    ...
    class Meta:
        model = PlayerStats
        fields = [
            'user_character_name', 'level', 'credits_earned', 
            'experience_points', 'guild_name', 'pvp_kills', 
            'pvp_deaths', 'kd_ratio'
        ]