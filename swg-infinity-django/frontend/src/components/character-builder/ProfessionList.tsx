...
export function ProfessionList({ professions, selectedProfession, onSelectProfession }: ProfessionListProps) {
  const [filter, setFilter] = useState<'all' | 'basic' | 'elite' | 'hybrid' | 'force' | 'pilot'>('all');
  ...
  const professionCounts = {
    ...
    hybrid: professions.filter(p => p.type === 'hybrid').length,
      ...
      return (
        <>
          ...
          {groupedProfessions.hybrid.length > 0 && (
            <>
              <h4 className="text-sm font-medium text-[hsl(var(--swg-accent-gold))] mt-3 mb-1">Hybrid Professions ({groupedProfessions.hybrid.length})</h4>
  ...
  return (
    <div className="bg-[#068ba3] p-3 rounded-md">
      ...
      <div className="mb-3 grid grid-cols-2 gap-1 text-xs">
        {(['all', 'basic', 'elite', 'hybrid', 'force', 'pilot'] as const).map(filterType => (
          <button
            key={filterType}
