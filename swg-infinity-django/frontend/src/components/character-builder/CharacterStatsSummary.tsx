...
export function CharacterStatsSummary({
  ...
  // Calculate character build efficiency
  const efficiency = maxSkillPoints > 0 ? Math.round((usedSkillPoints / maxSkillPoints) * 100) : 0;
    ...
    <div className="bg-[#068ba3] p-3 rounded-md">
      ...
      <div className="bg-[#04444a] rounded p-3 mb-3">
        <div className="grid grid-cols-2 gap-2 text-sm">
          ...
          <div>
            <span className="text-[#6dd9eb]">Efficiency:</span>
            <span className={`ml-1 font-medium ${
              efficiency >= 90 ? 'text-green-400' :
              efficiency >= 70 ? 'text-yellow-400' : 'text-orange-400'
            }`}>
              {efficiency}%
            </span>
          </div>
          ...
          <div>
            <span className="text-[#6dd9eb]">Remaining:</span>
            <span className={`ml-1 font-medium ${
              remainingPoints > 50 ? 'text-green-400' :
              remainingPoints > 20 ? 'text-yellow-400' : 'text-red-400'
            }`}>
              {remainingPoints} SP
            </span>
          </div>
          ...
          {remainingPoints > 0 && (
            <div className="mt-3 pt-3 border-t border-[#056473]">
              ...
              <div className="text-xs text-[#b8dce3] space-y-1">
                ...
                {efficiency < 50 && (
                  <p>• Your build efficiency is low - consider completing more skill trees for better bonuses.</p>
                )}
